...
# =========================================================
# 🏎️ VIEWPORT CANVAS CONFIGURATION
# =========================================================
...
demo_mode = st.sidebar.toggle(
    ...
    value=False, 
    help="Force simulated telemetry trace visualization arrays."
)
...
def fetch_calibrated_telemetry(s_key, d_map, d_a, d_b, target_length, fallback_active):
    ...
    try:
        ...
        # FIXED: Synchronous Timeline Slicing Layer isolates absolute common boundaries.
        # This completely drops dead packet lines and stops the 70s delta leakage dead.
        ...
        # Validation gate throws out compilation values if a corrupted stream gap is detected
...
# =========================================================
# STATEFUL CACHING CORE (ELIMINATES CONSTANT DIMMING)
# =========================================================
state_key = f"{selected_year}_{selected_round}_{selected_session_label}_{driver_a}_{driver_b}_{demo_mode}"

if "last_state_key" not in st.session_state or st.session_state["last_state_key"] != state_key:
    ...
    if engine_status != "SUCCESS" or telemetry_a is None:
        ...
        speed_base = 280.0
        for i in range(14):
            corner_pos = (true_circuit_length / 15) * (i + 1) + np.random.uniform(-100, 100)
        ...
        delta_time = (np.cumsum(1 / (np.maximum(speed_a, 15) / 3.6)) - np.cumsum(1 / (np.maximum(speed_b, 15) / 3.6))) * 0.4
        
        st.session_state["telemetry_a"] = pd.DataFrame({'Distance': dist_baseline, 'Speed': speed_a, 'Throttle': throttle_a, 'Delta_Time': delta_time})
        st.session_state["telemetry_b"] = pd.DataFrame({'Distance': dist_baseline, 'Speed': speed_b, 'Throttle': throttle_b})
        st.session_state["lineage_integrity"] = "100% Emulated Core"
        st.session_state["engine_status"] = engine_status
    else:
        st.session_state["telemetry_a"] = telemetry_a
        st.session_state["telemetry_b"] = telemetry_b
        st.session_state["lineage_integrity"] = "100% Authentic API"
        st.session_state["engine_status"] = "SUCCESS"
        
    st.session_state["last_state_key"] = state_key

# Load stabilized variables from state memory cache layer
telemetry_a = st.session_state["telemetry_a"]
telemetry_b = st.session_state["telemetry_b"]
lineage_integrity = st.session_state["lineage_integrity"]
engine_status = st.session_state["engine_status"]

# =========================================================
# 📑 EXECUTIVE SUMMARY INSIGHTS PANEL (CLEAN RENDER)
# =========================================================
if lineage_integrity == "100% Authentic API":
    st.success("✅ **Dashboard Pipeline Mode:** Live F1 Server Pipeline Sync Operational")
else:
    st.info("🖥️ **Dashboard Pipeline Mode:** Sandbox Emulator Interface Active")
...
# FIXED: Removed the messy, flashing alert warnings completely from user viewport view.
if lineage_integrity == "100% Emulated Core" and not demo_mode:
    st.caption("💡 **Recruiter Portfolio Review Tip:** Switch the sidebar season to **2024** to instantly pull up 100% pristine, high-density telemetry matching curves directly from archived track repositories.")
...
fig.update_layout(height=850, template="plotly_dark", showlegend=True, plot_bgcolor='#0E1117', paper_bgcolor='#0E1117',
                  xaxis3_title="Distance Traveled (Meters)", yaxis_title="Velocity (km/h)", yaxis2_title="Throttle %", yaxis3_title="Delta (Seconds)")
fig.update_xaxes(gridcolor='#222933', zerolinecolor='#444d56')
fig.update_yaxes(gridcolor='#222933', zerolinecolor='#444d56')
st.plotly_chart(fig, use_container_width=True)

# =========================================================
# 📘 COMPREHENSIVE FIELD MANUAL GUIDE (100% PRESERVED)
# =========================================================
st.markdown("---")
