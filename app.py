...
# 2. Sidebar - Chronological Round Selection
st.sidebar.title("🏎️ Portfolio Control Panel")
year = st.sidebar.selectbox("Select Season", [2026, 2025, 2024])

# Fetch meetings and filter for Rounds only (excluding testing)
meetings = fetch_api(f"https://api.openf1.org/v1/meetings?year={year}")
# Filter: Keep only meetings with a numeric round (removes testing/unassigned)
valid_meetings = [m for m in meetings if isinstance(m.get('round'), int)]
meetings_sorted = sorted(valid_meetings, key=lambda x: x['round'])

# Dropdown with "Round X: Name" format
meeting_options = {f"Round {m['round']}: {m['meeting_name']}": m['meeting_key'] for m in meetings_sorted}
...
if st.sidebar.button("🚀 Load Data"):
    # Mock data for stable UI rendering
...
if st.session_state.telemetry:
    ...
    for title, key in [("Velocity Profile (Speed)", "speed"), ("Throttle Map", "throttle"), ("Delta Time (Gap)", "delta")]:
        ...
        if key == 'delta':
            ...
            fig.add_trace(go.Scatter(y=st.session_state.telemetry[key][0], name=d1, line=dict(color='#00FFFF', width=2)))
            fig.add_trace(go.Scatter(y=st.session_state.telemetry[key][1], name=d2, line=dict(color='#FF00FF', width=2)))
