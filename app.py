...
def fetch_api(endpoint, params=None):
    res = requests.get(f"https://api.openf1.org/v1/{endpoint}", params=params)
            ...
            with st.spinner(f"Analyzing {d1} vs {d2}..."):
                ...
                if not df_a.empty and not df_b.empty:
                    ...
                    with st.expander("📖 Guide"):
                        st.write("This dashboard provides high-fidelity telemetry comparisons using the OpenF1 API. Data is filtered to the fastest completed lap of the session.")
                else:
