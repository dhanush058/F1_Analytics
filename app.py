...
# 2. Data Fetcher with Error Handling
...
# 3. Sidebar: Chronological Selection
...
# Fetch meetings, sort by dateStart to ensure chronological order
meetings = fetch_api(f"https://api.openf1.org/v1/meetings?year={year}")
meetings_sorted = sorted(meetings, key=lambda x: x['date_start'])

# Create ordered labels: Round 1: Australian Grand Prix
meeting_options = {f"Round {m['round_number']}: {m['meeting_name']}": m['meeting_key'] for m in meetings_sorted}
