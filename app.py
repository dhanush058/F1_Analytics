...
# 2. Styling (Red borders, Dark background)
st.markdown("""
    <style>
...
if st.session_state.telemetry:
    ...
    # Velocity
    st.write("### Velocity Profile (Speed)")
    fig_speed = go.Figure()
    fig_speed.add_trace(go.Scatter(y=data['speed'][0], name=d1, line=dict(color='#00FFFF', width=2)))
    fig_speed.add_trace(go.Scatter(y=data['speed'][1], name=d2, line=dict(color='#FF00FF', width=2)))
    fig_speed.update_layout(template="plotly_dark", plot_bgcolor='#0E1117', paper_bgcolor='#0E1117', height=300, margin=dict(t=30, b=30, l=30, r=30))
    st.plotly_chart(fig_speed, use_container_width=True, theme=None)
    
    # Throttle
    st.write("### Throttle Map")
    fig_throttle = go.Figure()
    fig_throttle.add_trace(go.Scatter(y=data['throttle'][0], name=d1, line=dict(color='#00FFFF', width=2)))
    fig_throttle.add_trace(go.Scatter(y=data['throttle'][1], name=d2, line=dict(color='#FF00FF', width=2)))
    fig_throttle.update_layout(template="plotly_dark", plot_bgcolor='#0E1117', paper_bgcolor='#0E1117', height=300, margin=dict(t=30, b=30, l=30, r=30))
    st.plotly_chart(fig_throttle, use_container_width=True, theme=None)
    
    # Delta
    st.write("### Delta Time (Gap to Baseline)")
    fig_delta = go.Figure()
    fig_delta.add_trace(go.Scatter(y=data['delta'], name="Gap", line=dict(color='#FFFF00', width=3)))
    fig_delta.update_layout(template="plotly_dark", plot_bgcolor='#0E1117', paper_bgcolor='#0E1117', height=300, margin=dict(t=30, b=30, l=30, r=30))
