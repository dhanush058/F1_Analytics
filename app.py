...
def get_telemetry(driver_api_name, s_key, drivers_df, track_name, session_name, year, is_sim=False, driver_id=1):
    if is_sim:
        ...
        speed = np.full(1000, base_vmax + (driver_seed % 6) - 3)
        throttle = np.full(1000, 100.0)
        return pd.DataFrame({'distance': dist_ref, 'speed': speed, 'throttle': throttle}), 90.0, track_length
    ...
    if not sessions.empty:
        ...
        if not drivers_data.empty:
            ...
            if not df_a.empty and not df_b.empty:
                ...
                fig.add_trace(go.Scatter(x=df_a['distance'], y=df_a['speed'], name=d1_display, line=dict(color=COLOR_A)), row=2, col=1)
                fig.add_trace(go.Scatter(x=df_b['distance'], y=df_b['speed'], name=d2_display, line=dict(color=COLOR_B)), row=2, col=1)
